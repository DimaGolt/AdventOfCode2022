...
void main(List<String> args) async {
  ...
  for (String line in file) {
    if (line.isEmpty) {
      ...
      var command = [
        int.parse(line.substring(regEx[0].start, regEx[0].end)),
        int.parse(line.substring(regEx[1].start, regEx[1].end)),
        int.parse(line.substring(regEx[2].start, regEx[2].end))
      ];
      int from = command[1] - 1;
      int to = command[2] - 1;
      for (int i = int.parse(command[0].toString()); i > 0; i--) {
